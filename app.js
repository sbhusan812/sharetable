...
const activityKey = 'sharetable-activity';

function getActivity() {
  try { return JSON.parse(localStorage.getItem(activityKey)) || []; } catch { return []; }
}
function recordActivity(label, detail, kind = 'action') {
  const activity = [{ label, detail, kind, time: new Date().toLocaleDateString([], { day: 'numeric', month: 'short' }) }, ...getActivity()].slice(0, 8);
  localStorage.setItem(activityKey, JSON.stringify(activity));
}
function renderDashboard() {
  const activity = getActivity();
  const shared = activity.filter(item => item.kind === 'shared').reduce((total, item) => total + Number(item.portions || 0), 0);
  const requests = activity.filter(item => item.kind === 'request').length;
  document.querySelector('#stat-shared').textContent = shared;
  document.querySelector('#stat-rescued').textContent = requests;
  document.querySelector('#stat-actions').textContent = activity.length;
  document.querySelector('#dashboard-intro').textContent = currentSession?.user ? `A quiet record of the good you are making, ${currentSession.user.user_metadata?.display_name || currentSession.user.email.split('@')[0]}.` : 'Your sharing history and next steps, all in one place.';
  document.querySelector('#activity-list').innerHTML = activity.length ? activity.map(item => `<div class="activity-item"><span class="activity-icon">${item.kind === 'shared' ? '↗' : item.kind === 'request' ? '⌂' : '✓'}</span><p><strong>${item.label}</strong><br /><small>${item.detail} · ${item.time}</small></p></div>`).join('') : '<p class="empty-activity">Your first good action will show up here.</p>';
}
function openDashboard() { renderDashboard(); toggleModal('#dashboard-modal', true); }
...
async function claimListing(listingId, title) {
  ...
  recordActivity('Food requested', title, 'request');
...
document.querySelector('#share-form').addEventListener('submit', async event => {
  ...
  recordActivity('Surplus shared', `${data.get('portions')} portions of ${data.get('title')}`, 'shared');
...
document.querySelectorAll('[data-open-login]').forEach(button => button.addEventListener('click', () => currentSession ? openDashboard() : toggleModal('#login-modal', true)));
...
document.querySelector('#volunteer-form').addEventListener('submit', event => { event.preventDefault(); const data = new FormData(event.target); recordActivity('Volunteer application sent', data.get('role')); toggleModal('#volunteer-modal', false); showToast('Application sent. We will review your identity and references.'); });
...
document.querySelector('#bulk-form').addEventListener('submit', event => { event.preventDefault(); const data = new FormData(event.target); recordActivity('Large donation listed', `${data.get('portions')} portions from ${data.get('source')}`, 'shared'); toggleModal('#bulk-modal', false); showToast('Verified NGOs near you have been notified'); });
document.querySelector('[data-close-dashboard]').addEventListener('click', () => toggleModal('#dashboard-modal', false));
document.querySelector('#dashboard-modal').addEventListener('click', event => { if (event.target.id === 'dashboard-modal') toggleModal('#dashboard-modal', false); });
document.querySelector('[data-dashboard-share]').addEventListener('click', () => { toggleModal('#dashboard-modal', false); setModal(true); });
document.querySelector('[data-dashboard-volunteer]').addEventListener('click', () => { toggleModal('#dashboard-modal', false); toggleModal('#volunteer-modal', true); });
document.querySelector('[data-clear-activity]').addEventListener('click', () => { localStorage.removeItem(activityKey); renderDashboard(); showToast('Local activity history cleared'); });
renderFood();
loadRemoteFood();
